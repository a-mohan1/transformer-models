...
class AttentionBlock(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        x = self.attention(inputs, inputs, return_attention_scores=False) 
    ...
    def get_attention_scores(self, inputs):
        _, scores = self.attention(inputs, inputs, training=False, return_attention_scores=True) 
        return scores
    
class AttClassifier(tf.keras.Model):
    def __init__(self, name='AttClassifier', time2vec_dim=2, num_heads=2, key_dim=64, ff_dim=32, 
                num_attlayers=1, dropout=0, n_dense=128, num_classes=2, **kwargs):
...
def create_transformer_classifier_1lead(max_length=1500, num_classes=2, time2vec_dim=8, 
    ...
    outputs = AttClassifier(time2vec_dim=time2vec_dim, num_heads=num_heads, key_dim=key_dim,
                            ...
                            n_dense=n_dense, num_classes=num_classes)(inputs)
...
class ViT_AttentionBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        # Self-attention
        x = self.attn_layer(x, x, return_attention_scores=False)
        x = self.add([inputs, x])
    ...
    def get_attention_scores(self, inputs):
        ...
        _, scores = self.attn_layer(x, x, training=False, return_attention_scores=True)
        return scores
    
@keras.saving.register_keras_serializable(package='ViTClassifier')
class ViTClassifier(tf.keras.layers.Layer):
    def __init__(self, num_classes=2, patch_size=30, num_att_layers=1, mlp_units=[128], num_heads=2, embedding_dim=10, dropout=0.0, **kwargs):
    ...
    def get_last_selfattention(self, inputs):
        x = self.vit_embeddings(inputs, training=False)
        # Run through all but last layer
        for att_layer in self.vit_attention_layers[:-1]:
            x = att_layer(x, training=False)
        # Attention scores from last layer
        scores = self.vit_attention_layers[-1].get_attention_scores(x)
        return scores
...
def create_vit_classifier_2leads(max_length=1500, num_classes=2, patch_size=30, 
    ...
    inputs = tf.keras.layers.Concatenate(axis=-1)([main_signal, sec_signal])
    outputs = ViTClassifier(num_classes=num_classes, patch_size=patch_size, 
                            num_att_layers=num_att_layers, mlp_units=mlp_units, 
                            num_heads=num_heads, embedding_dim=embedding_dim, 
                            dropout=dropout)(inputs)
...
# TRANSFORMER WITH CLASSIFIER TOKEN
...
def plot_attn_signal(df_test: pd.DataFrame, attention_scores: tf.Tensor, max_length: int, class_labels: np.array) -> plt.subplots:
    ...
    for i in range(len(df_test)):
        ...
        attn = tf.image.resize(attn, (1, max_length))[0,0] # tf.image.resize works on 2 middle dims
        # Take attention values for signal length, excluding zero-padding in front
        signal_length = df_test.loc[i, 'signal_length']
        attn_siglen = attn[-signal_length:, :]
